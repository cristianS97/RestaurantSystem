...
export default function BillingPage() {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                ...
                setOrders(data.filter(order => order.status === 'served'));
                setLoading(false);
            } catch (err) {
    ...
    if (loading) return <div>Cargando órdenes servidas...</div>;

    return (
        <div className="p-4">
            <h2 className="text-xl font-bold mb-4">Caja - Órdenes servidas</h2>
